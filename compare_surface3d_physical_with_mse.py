...
def main():
    ...
    ap.add_argument("--mse_out", type=str, default="", help="CSV path to save per-time spatial MSEs")
    args = ap.parse_args()
    ...
    for r, tval in enumerate(tlist):
        ...
        # spatial MSEs on the disk (ignore NaNs outside)
        mse_p = float(np.nanmean((Yp - WT) ** 2))
        mse_s = float(np.nanmean((Ys - WT) ** 2))
        mse_rows.append((tval, mse_p, mse_s))
        print(f"[MSE] t={tval:g}  physical={mse_p:.6e}  ssv={mse_s:.6e}")
    ...
    # save MSEs
    mse_path = args.mse_out.strip() or (os.path.splitext(args.out)[0] + "_mse.csv")
    try:
        ...
        with open(mse_path, "w", encoding="utf-8") as f:
            f.write("t,mse_physical,mse_ssv\n")
            for tval, mp, ms in mse_rows:
                f.write(f"{tval},{mp},{ms}\n")
        print(f"[compare_surface3d_physical] saved MSEs to {mse_path}")
    except Exception as e:
        print(f"[compare_surface3d_physical] WARNING: failed to save MSEs: {e}")
